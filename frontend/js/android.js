  ...
  api.androidVersions().then(data=>{
    ...
    tbody.innerHTML=versions.map((v,i)=>{
      ...
      return `<tr data-aos="fade-up" data-aos-delay="${Math.min(i,12)*30}">
        ...
        <td style="color:let(--muted)">${esc(v.codename||'—')}</td>
        <td style="font-family:let(--mono);color:let(--info)">${esc(String(v.api_level||'—'))}</td>
        <td style="color:let(--muted)">${esc(String(v.release_year||'—'))}</td>
