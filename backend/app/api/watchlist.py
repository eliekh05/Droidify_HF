...
GET  /api/watchlist          — list saved devices (with full device info)
...
async def list_watchlist(request: Request):
    """Return saved devices with basic info fetched from the device API."""
    ...
    # Fetch basic info for each saved device
    devices = []
    for codename in codenames:
        try:
            result = await get_devices(q=codename, limit=1, offset=0)
            devs = result.get("devices", [])
            # Find exact codename match
            ...
            if match:
                ...
                # Device not found in scraper — keep codename only
                devices.append({"codename": codename, "model": None})
        except Exception:
            devices.append({"codename": codename, "model": None})

    return JSONResponse({"watchlist": devices, "total": len(devices)})
